...
class BatchNormalizationLayer(StochasticLayerWithParameters):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        X_hat = (inputs - mean) / np.sqrt(variance + self.epsilon)
        input_norm = self.gamma.reshape(mean.shape) * X_hat +self.beta.reshape(mean.shape)
        
        self.cache = variance, mean, X_hat
        return input_norm
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        variance, mean, X_hat = self.cache

        N = inputs.shape[0]
        dout_ = self.gamma * grads_wrt_outputs
        dvar = np.sum(dout_ * (inputs - mean) * -0.5 * (variance + self.epsilon) ** -1.5, axis=0)
        dx_ = 1 / np.sqrt(variance + self.epsilon)
        dvar_ = 2 * (inputs - mean) / N

        di = dout_ * dx_ + dvar * dvar_
        dmean = -1 * np.sum(di, axis=0)
        dmean_ = np.ones_like(inputs) / N
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        dgamma = np.sum(grads_wrt_outputs * X_hat, axis=0)
...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        H_new = self.input_dim_1 - self.kernel_dim_1 + 1

        W_new = self.input_dim_2 - self.kernel_dim_2 + 1

        N = inputs.shape[0]

        outputs = np.zeros((N, self.num_output_channels, H_new, W_new))
        ...
        for n in range(N):
            ...
            for j in range(self.num_output_channels):
                ...
                for k in range(self.num_input_channels):
                    ...
                    outputs[n, j] += convolve2d(inputs[n, k], self.kernels[j, k], mode='valid')

                outputs[n, j] += self.biases[j]
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        grads_wrt_inputs = np.zeros(
            ...
            (N, self.num_input_channels, self.input_dim_1, self.input_dim_2))
        ...
        for n in range(N):
            ...
            for j in range(self.num_output_channels):
                ...
                for k in range(self.num_input_channels):
                    ...
                    grads_wrt_inputs[n, k] += correlate2d(

                        padded_grads_wrt_outputs[n, j], self.kernels[j, k], mode='valid')
        ...
        for n in range(N):
            ...
            for j in range(self.num_output_channels):
                ...
                for k in range(self.num_input_channels):
                    ...
                    grads_wrt_kernels[j, k] += correlate2d(

                        grads_wrt_outputs[n, j], inputs[n, k], mode='valid')
