...
class LeakyReluLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        For inputs `x` and outputs `y` this corresponds to `y = max(0, x)`.
        """
        outputs = 0.01*inputs*(inputs<=0) + inputs*(inputs>0)
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        gradients with respect to the layer inputs.
        """
        gradients = grads_wrt_outputs*(inputs>0) + 0.01*grads_wrt_outputs*(inputs<=0)
        return gradients
...
class ELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        For inputs `x` and outputs `y` this corresponds to `y = max(0, x)`.
        """
        outputs = (np.exp(inputs)-1)*(inputs<=0)+(inputs)*(inputs>0)
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        gradients with respect to the layer inputs.
        """
        gradients = np.exp(inputs)*grads_wrt_outputs*(inputs<=0)+grads_wrt_outputs*(inputs>0)
        return gradients
...
class SELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        α01 = 1.6733 
        λ01 = 1.0507
        outputs = λ01*α01*(np.exp(inputs)-1)*(inputs<=0)+λ01*inputs*(inputs>0) 
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        α01 = 1.6733 
        λ01 = 1.0507
        gradients = λ01*α01*np.exp(inputs)*grads_wrt_outputs*(inputs<=0) + λ01*grads_wrt_outputs*(inputs>0) 
        return gradients
